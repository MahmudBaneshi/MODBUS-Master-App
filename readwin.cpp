...
ReadWin::~ReadWin()
{
    delete ui;
    delete readDevTable;
    delete dataTypeDelegate;
    delete noEditDelegate;
    delete spinDelegate;
    delete lEditeDelegate_2;
    delete lEditeDelegate_3;
    delete lEditeDelegate;
}

void ReadWin::updateModel(int first)
{
    if(sqlDb.open()){
        if(first){
            readDevTable = new QSqlTableModel(this, sqlDb);
            readDevTable->setTable("READ_DEV");
            readDevTable->select();
...
void ReadWin::on_comboBox_activated(const QString &)
{
    updateModel(0);
}
/*  add  */
void ReadWin::on_pushButton_clicked()
{
    if(ui->comboBox->currentText() == "NONE"){
        QMessageBox msg;
...
/*  remove  */
void ReadWin::on_pushButton_2_clicked()
{
